function game1() {
    ...
    function renderScoreHud() {
        ...
        cubeCountNode.innerText = `CUBES ECRASE: ${state.game.cubeCount}`;
    }
    ...
    function renderSlowmoStatus(percentRemaining) {
        ...
        slowmoBarNode.style.transform = `scaleX(${percentRemaining.toFixed(
            3
        )})`;
    }
    ...
    function renderMenus() {
        ...
        switch (state.menus.active) {
            ...
            case MENU_SCORE:
                ...
                if (isNewHighScore()) {
                    highScoreLblNode.textContent = "Nouveau Meilleur Score!";
                } else {
                    highScoreLblNode.textContent = `Meilleur Score: ${formatNumber(
                        getHighScore()
                    )}`;
                }
    ...
    function draw(ctx, width, height, viewScale) {
        ...
        allPolys.forEach((p) => {
            ...
            if (!p.wireframe) {
                const normalLight =
                    p.normalWorld.y * 0.5 + p.normalWorld.z * -0.5;
                const lightness =
                    normalLight > 0
                        ? 0.1
